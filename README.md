...
The exiftool version matters here.
12.57, which Debian bookworm packages, reads only part of the acquisition record on longer clips: three items where 13.59 reads fourteen on the same bytes, with `CaptureGammaEquation` absent from the three.
The symptom is indistinguishable from footage that carries no picture profile, so the image installs a pinned exiftool from source instead of the distribution package, and the service logs the version it found on every run.
